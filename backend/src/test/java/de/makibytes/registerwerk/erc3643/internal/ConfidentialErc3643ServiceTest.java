...
/**
 * Unit test for the finding #7 (Phase 9) fail-closed fix: deploying a confidential ERC-3643 with
 * no configured viewer must abort rather than silently proceed with a permanently undecryptable
 * register entry.
 */
@ExtendWith(MockitoExtension.class)
class ConfidentialErc3643ServiceTest {
    ...
    void deploy_throws_whenNoViewersConfigured() {
        lenient().when(assetLookupPort.findById(assetId)).thenReturn(Optional.of(
                new AssetLookupPort.AssetInfo(assetId, "Test Confidential Security", "DE000TEST002",
                        TokenStandard.CONF_ERC3643, "ETHEREUM", "TESTNET", UUID.randomUUID(), "AN-2", "ACTIVE")));
        lenient().when(contractAddressConfig.requireConfidentialFactory(any())).thenReturn("0x" + "aa".repeat(20));
        lenient().when(contractAddressConfig.requireConfidentialIdentityRegistry(any())).thenReturn("0x" + "cc".repeat(20));
        lenient().when(contractAddressConfig.confidentialComplianceOrZero(any())).thenReturn("0x" + "00".repeat(20));
        ...
        ConfidentialErc3643Service service = new ConfidentialErc3643Service(
