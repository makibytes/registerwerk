              ...
              @for (trade of trades; track trade.id) {
                <mat-expansion-panel class="trade-panel">
                  ...
                  @if (trade.status === 'PENDING_OPEN_SETTLEMENT') {
                    <div class="action-strip"><div><strong>Opening settlement</strong><span>{{ canOpenSettle(trade) ? 'Each recipient confirms the leg they received.' : 'Scheduled for ' + (trade.startDate | date:'mediumDate') + '. Confirmation opens on the start date.' }}</span></div><mat-form-field appearance="outline"><mat-label>Settlement reference</mat-label><input matInput [(ngModel)]="references[trade.id]" [disabled]="!canOpenSettle(trade)"></mat-form-field>@if (trade.borrower && !trade.openCashConfirmed) {<button type="button" mat-flat-button color="primary" [disabled]="!canOpenSettle(trade)" (click)="confirmOpen(trade, 'CASH')">Confirm cash received</button>}@if (!trade.borrower && !trade.openCollateralConfirmed) {<button type="button" mat-flat-button color="primary" [disabled]="!canOpenSettle(trade)" (click)="confirmOpen(trade, 'COLLATERAL')">Confirm collateral received</button>}<div class="leg-state"><mat-icon>{{ trade.openCashConfirmed ? 'check_circle' : 'radio_button_unchecked' }}</mat-icon>Cash <mat-icon>{{ trade.openCollateralConfirmed ? 'check_circle' : 'radio_button_unchecked' }}</mat-icon>Collateral</div></div>
                  }
                  @if (trade.status === 'MARGIN_CALL') {
                    <div class="alert-strip"><mat-icon>warning</mat-icon><div><strong>Margin call: {{ trade.marginCallAmount | number:'1.2-2' }} {{ trade.cashCurrency }}</strong><span>Due {{ trade.marginCallDueAt | date:'short' }}</span></div>@if (trade.borrower) {<mat-form-field appearance="outline"><mat-label>Transfer reference</mat-label><input matInput [(ngModel)]="references[trade.id]"></mat-form-field><button type="button" mat-flat-button color="primary" (click)="satisfyMargin(trade)">Confirm margin delivered</button>}</div>
                  }
