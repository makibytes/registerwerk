...
@Component({
  ...
  styles: [`
    ...
    .filter-label { font-size: 11px; font-weight: 600; letter-spacing: 0.5px; text-transform: uppercase; color: var(--rw-text-muted); }
    .filter-chip {
      display: inline-flex; align-items: center; gap: 4px; padding: 4px 12px; border-radius: 20px;
      font-size: 12px; font-weight: 600; cursor: pointer; border: 1px solid var(--rw-border);
      background: transparent; color: var(--rw-text-secondary); transition: all 0.15s ease;
    ...
    <div class="page-container">
      ...
      <div class="content-card">
        @if (filtered().length === 0) {
          ...
          <table>
            <thead>
              <tr><th>Name</th><th>Type</th><th>Address</th><th>Risk</th><th>Notes</th><th></th></tr>
            </thead>
            <tbody>
              @for (ep of filtered(); track ep.id) {
                <tr>
                  <td class="name-cell">{{ ep.name }}</td>
                  <td><span class="type-chip" [class.wallet]="ep.addressType === 'WALLET'" [class.contract]="ep.addressType === 'CONTRACT'">{{ ep.addressType === 'WALLET' ? 'Wallet' : 'Contract' }}</span></td>
                  <td>
                  ...
                  <td>
                    @if (ep.riskLevel) {
                      <span class="risk-chip" [class.low]="ep.riskLevel === 'LOW'" [class.medium]="ep.riskLevel === 'MEDIUM'" [class.high]="ep.riskLevel === 'HIGH'">{{ ep.riskLevel }}</span>
                    } @else {
...
export class EndpointsListComponent implements OnInit {
  ...
  openCreate(): void {
    this.dialog.open<EndpointFormDialogComponent, EndpointFormDialogData, EndpointFormDialogResult>(
      EndpointFormDialogComponent, { width: '480px', data: { mode: 'create' } }
    ).afterClosed().subscribe((result: EndpointFormDialogResult | undefined) => {
      if (!result) return;
      this.endpointService.createEndpoint(result as EndpointCreateRequest).subscribe({
        next: () => { this.load(); this.snackBar.open('Endpoint created', 'OK', { duration: 3000 }); },
        error: (e: { error?: { message?: string } }) => this.snackBar.open(e.error?.message ?? 'Failed to create endpoint', 'OK', { duration: 4000 }),
      });
    });
  }
  ...
  openEdit(ep: Endpoint): void {
    this.dialog.open<EndpointFormDialogComponent, EndpointFormDialogData, EndpointFormDialogResult>(
      EndpointFormDialogComponent, { width: '480px', data: { mode: 'edit', endpoint: ep } }
    ).afterClosed().subscribe((result: EndpointFormDialogResult | undefined) => {
      if (!result) return;
      this.endpointService.updateEndpoint(ep.id, result as EndpointUpdateRequest).subscribe({
        next: () => { this.load(); this.snackBar.open('Endpoint updated', 'OK', { duration: 3000 }); },
        error: (e: { error?: { message?: string } }) => this.snackBar.open(e.error?.message ?? 'Failed to update endpoint', 'OK', { duration: 4000 }),
      });
    });
  }
  ...
  confirmDelete(ep: Endpoint): void {
    ...
    this.endpointService.deleteEndpoint(ep.id).subscribe({
      next: () => { this.load(); this.snackBar.open('Endpoint deleted', 'OK', { duration: 3000 }); },
      error: (e: { error?: { message?: string } }) => this.snackBar.open(e.error?.message ?? 'Failed to delete endpoint', 'OK', { duration: 4000 }),
    });
  }
