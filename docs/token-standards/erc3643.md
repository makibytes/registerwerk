...
Modules are configured **post-deployment** via `POST /compliance-modules` (REGISTRY_ADMIN)
— every newly deployed suite starts with an empty `ModularCompliance`, no investor-count/
jurisdiction/cooldown enforcement, until an operator explicitly attaches modules. This should
happen before the asset is opened for subscription if those rules are meant to apply from the
first investor onward.
