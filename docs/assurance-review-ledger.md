...
| M0-INDEXER-B | Add provisional/final cursors, reorg rollback and direct-chain reconciliation | Proposed, changes noted (unreviewed) | IN_PROGRESS | `token_transfer` now carries `finality_status` (PROVISIONAL/FINAL/ORPHANED, never deleted) plus `block_hash`; `ReorgGuard` re-verifies the provisional window every tick by re-fetching a fresh canonical hash/status and comparing against what was recorded, orphaning and rewinding the cursor on mismatch. Implemented for EVM (graph-node `_meta` block hash) and Starknet (`ACCEPTED_ON_L1` vs `ACCEPTED_ON_L2`/REJECTED via the tx's own `finality_status`, not a block hash); Solana/Stellar/Canton are documented as final-on-write for this codebase's indexing path (no probabilistic-finality primitive at that layer) rather than left silently unhandled. Submission-side (`blockchain_transaction`, deployment confirmation) now also persists and re-verifies a block hash before advancing past PENDING. Historical rows written before this migration are backfilled FINAL by default and were never retroactively re-verified against a fork-point hash — a documented limitation, not a retroactive finality proof. No trusted-RPC/quorum policy exists yet (a single configured RPC endpoint's `_meta`/status response is trusted as-is), and a reorg deeper than the configured confirmation-depth policy is detected and logged CRITICAL but not otherwise specially handled beyond what the shallow-reorg path already does. |
...
| F0-007 | Chain finality and deployed bytecode/admin/configuration reconciliation | Blocked — decision required | BLOCKED_DECISION | M0-INDEXER-B's provisional/final/orphan plumbing and per-tick re-verification against a freshly re-fetched hash/status now exist and are covered by unit tests exercising the fork-detection algorithm directly (a live multi-node RPC quorum and a real chain reorg were not exercised end-to-end in this environment). Still unresolved and still blocking: a trusted-RPC/quorum policy (today a single configured endpoint's response is trusted), deployed bytecode/proxy/admin/owner/key reconciliation, and the legal-reliance policy for when a technical finality signal may be relied on for register purposes |
