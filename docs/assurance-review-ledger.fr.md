...
| Relais confidentiel | Lint réussi ; 33/33 tests réussis | L'audit de dépendances a signalé 21 constats de gravité élevée ; le fichier de verrouillage était absent avant l'installation de référence |
| Sous-graphe EVM | 16 ABI de contrats / 71 gestionnaires, 25 entités de statut de projection, moteur de rendu multi-instance, génération de code, tous les builds de correspondance et les portes de déploiement étiquetées passent | La réorganisation/finalité, la relecture/le rapprochement en direct et l'identité du bytecode RPC restent en attente ; l'audit de dépendances rapporte encore 45 constats, dont deux critiques |
| Applications Angular opérateur/investisseur | Les commandes CI échouent | Les deux workflows appellent une cible de lint manquante ; Karma attend `karma-jasmine`, qui est manquant ; aucun fichier de spécification n'a été trouvé |
| Documentation Docusaurus | Les builds de production anglais et allemand réussissent après corrections de véracité/de build | L'audit de dépendances a tout de même signalé 37 constats, dont deux critiques |
