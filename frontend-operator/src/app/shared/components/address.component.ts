...
/**
 * Displays a blockchain address consistently across the operator frontend.
 *
 * - If `name` is provided it is shown directly (no resolution call).
 * - If `name` is absent the address is resolved via AddressResolutionService
 *   (batched, cached) and the resolved name is shown when available.
 * - On hover, unresolved addresses show a bookmark-add button that opens the
 *   endpoint creation dialog so users can add the address to their address book.
 * - Copy button and optional explorer link are always present.
 */
@Component({
