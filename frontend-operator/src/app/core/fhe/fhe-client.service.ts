...
/**
 * Browser-side Zama Relayer SDK integration — the piece that makes confidential-token decryption
 * genuinely available to the registry operator and an independent auditor, not just something
 * the backend does headlessly: connect a viewer wallet here (one already granted decrypt rights
 * via `TokenAdminService.confidentialAddViewer` or the deploy-time `initialViewers`) and reveal
 * any holder's balance directly against Zama's real relayer — see
 * `ConfidentialViewerPanelComponent`. Mirrors `frontend-customer`'s identical service; see that
 * file for the fuller rationale on why the backend/`zama-relayer` sidecar is never in this path.
 *
 * <p>Uses the connected wallet's own EIP-1193 provider as the SDK's `network` parameter — not a
 * backend-supplied RPC URL — so the browser never needs the registry's own (possibly paid/keyed)
 * RPC endpoint.
 *
...
export class FheClientService {
  ...
  /**
   * Encrypts `value` client-side for a confidential contract call. Included for completeness
   * (mirrors the customer app's service) — the operator portal's own confidential mint/force-burn
   * actions go through the backend's `zama-relayer` sidecar instead (there is no browser/wallet in
   * those flows by design; see `TokenAdminService.confidentialMint`/`confidentialForceBurn`).
   */
  async encrypt64(
  ...
  /**
   * Reveals ONE ciphertext handle by having the connected viewer wallet sign the Zama KMS's
   * `UserDecryptRequestVerification` EIP-712 authorization — succeeds only if the connected
   * address has actually been granted decrypt rights (a registered viewer — see
   * `ConfidentialERC20`'s viewer-ACL note). A wallet with no such grant gets a real KMS rejection,
