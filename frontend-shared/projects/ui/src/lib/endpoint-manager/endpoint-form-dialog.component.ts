...
export interface EndpointFormDialogData {
  mode: 'create' | 'edit';
  endpoint?: Endpoint;
}
...
@Component({
  selector: 'rw-endpoint-form-dialog',
  standalone: true,
  imports: [
    CommonModule,
    FormsModule,
  ...
  `],
  template: `
    <h2 mat-dialog-title>{{ isEdit ? 'Edit endpoint' : 'New endpoint' }}</h2>
    ...
    <mat-dialog-content>
      <div class="dialog-body">
        ...
        <div class="field-group">
          ...
          <mat-radio-group [(ngModel)]="form.addressType" [disabled]="isEdit" style="display:flex;gap:16px">
        ...
        <mat-form-field appearance="outline" class="address-field">
          ...
          <input matInput
                 ...
                 autocomplete="off">
          @if (isEdit) {
            <mat-hint>Address cannot be changed after creation</mat-hint>
          }
        </mat-form-field>
        ...
        <mat-form-field appearance="outline">
          ...
          <textarea matInput [(ngModel)]="form.notes" rows="3" maxlength="500"
...
export class EndpointFormDialogComponent {
  ...
  form: {
    ...
    name: string;
    notes: string;
    riskLevel: EndpointRiskLevel | undefined;
  } = {
    addressType: this.data.endpoint?.addressType ?? 'WALLET',
    address:     this.data.endpoint?.address     ?? '',
    name:        this.data.endpoint?.name        ?? '',
    notes:       this.data.endpoint?.notes       ?? '',
    riskLevel:   this.data.endpoint?.riskLevel,
  };

  canSave(): boolean {
    return this.form.name.trim().length > 0 && (this.isEdit || this.form.address.trim().length > 0);
  }

  save(): void {
    if (!this.canSave()) return;
    if (this.isEdit) {
      const req: EndpointUpdateRequest = {
      ...
      const req: EndpointCreateRequest = {
